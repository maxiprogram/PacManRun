...
QVector2D ManagerTileMap::GetTileIJ(QVector3D pos)
{
    ...
    res.setY(qFloor(qAbs(all_height-pos.y())/tile_height));
    return res;
}
...
Layer* ManagerTileMap::GetLayer(QString layer_name)
{
    Layer* layer = 0;
    for (int i=0; i<list_layer.size(); i++)
    {
...
bool ManagerTileMap::CollisionY(QString layer_name, QVector3D& pos, QRectF bound, QVector2D dir)
{
    //ground = false;
    bool flag = false;
    Layer* layer = 0;
